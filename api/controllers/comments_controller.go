...
func (server *Server) CreateComment(c *gin.Context) {
	...
	// enter the userid and the workouid. The comment body is automatically passed
	comment.UserID = uid
	comment.WorkoutID = pid
...
func (server *Server) UpdateComment(c *gin.Context) {
	...
	err = server.DB.Debug().Model(models.Workout{}).Where("id = ?", pid).Take(&origComment).Error
	if err != nil {
	...
	comment.ID = origComment.ID //this is important to tell the model the workout id to update, the other update field are set above
	comment.UserID = origComment.UserID
	comment.WorkoutID = origComment.WorkoutID
