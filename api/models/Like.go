...
type Like struct {
	...
	WprkoutID uint64    `gorm:"not null" json:"workout_id"`
...
func (l *Like) SaveLike(db *gorm.DB) (*Like, error) {
	...
	// Check if the auth user has liked this workout before:
	err := db.Debug().Model(&Like{}).Where("workout_id = ? AND user_id = ?", l.WorkoutID, l.UserID).Take(&l).Error
	if err != nil {
		if err.Error() == "record not found" {
			// The user has not liked this workout before, so lets save incomming like:
...
func (l *Like) GetLikesInfo(db *gorm.DB, pid uint64) (*[]Like, error) {
	...
	likes := []Like{}
	err := db.Debug().Model(&Like{}).Where("workout_id = ?", pid).Find(&likes).Error
	if err != nil {
		return &[]Like{}, err
	}
	return &likes, err
...
//When a workout is deleted, we also delete the likes that the workout had
...
func (l *Like) DeleteWorkoutLikes(db *gorm.DB, pid uint64) (int64, error) {
	likes := []Like{}
	db = db.Debug().Model(&Like{}).Where("workout_id = ?", pid).Find(&likes).Delete(&likes)
	if db.Error != nil {
		return 0, db.Error
	}
	return db.RowsAffected, nil
