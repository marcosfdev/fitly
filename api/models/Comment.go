...
func (c *Comment) GetComments(db *gorm.DB, pid uint64) (*[]Comment, error) {
	...
	comments := []Comment{}
	err := db.Debug().Model(&Comment{}).Where("workout_id = ?", pid).Order("created_at desc").Find(&comments).Error
	if err != nil {
		return &[]Comment{}, err
	}
	if len(comments) > 0 {
...
//When a Workout is deleted, we also delete the comments that the Workout had
func (c *Comment) DeleteWorkoutComments(db *gorm.DB, pid uint64) (int64, error) {
	comments := []Comment{}
	db = db.Debug().Model(&Comment{}).Where("workout_id = ?", pid).Find(&comments).Delete(&comments)
	if db.Error != nil {
		return 0, db.Error
	}
	return db.RowsAffected, nil
